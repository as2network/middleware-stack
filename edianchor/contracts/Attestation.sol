pragma solidity 0.4.24 || 0.4.25 || 0.4.26 <0.5.0;
...
contract AllowanceAndOwnershipContract is ERC721 {
    ...
    function disapproveAll(address _to) public {
        require(_to != msg.sender);
...
contract AnchoringContract is Ownable, ERC721BasicTokenContract {
    ...
    uint256 docRefCounter = 0; // This will be used as unique auto incremented attestationId
    mapping(uint256 => Version[]) versionOf; // mapping(attestationId => Version[])
    mapping(bytes32 => AttestationReference) attestationReference; // mapping(referenceId => AttestationReference)

    event AttestationSaved(
    ...
    */
    function _save(
        ...
        Version[] storage versions = versionOf[attestationId];
        uint256 versionsLength = versions.push(version);
        uint256 versionId = versionsLength - 1;
        versionOf[attestationId] = versions;
    ...
    */
    ...
    /*
        Accept referenceId of any previous track of a attestation and save the new version of that attestation using given data.
    */
    function updateAttestation(
        ...
        uint256 attestationId = attestationReference[referenceId].attestationId;
        return _save(attestationId, hash, description);
    }
